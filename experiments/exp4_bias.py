"""
does hardware heterogeneity induce per-class BIAS -- and only when the
...
BASE  = "configs/exp4_bias_verified.yaml"
...
def select_poor_and_disadvantaged(dist, frac, seed):
    ...
    n_poor = max(1, min(n_poor, len(actual_owners) - 1))  
    ...
    for seed in SEEDS:
        ...
        for cond in CONDITIONS:
            ...
            rec = dict(condition=cond, seed=seed, global_acc=float(acc),
                       ...
                       poor_reliability=POOR_RELIABILITY,
...
def plot_bias(out_root=f"./outputs/{EXPERIMENT}"):
    ...
    gap = {c: [] for c in conds}         
    dacc = {c: [] for c in conds}        
    racc = {c: [] for c in conds}         
    gacc = {c: [] for c in conds}        
    for r in records:
    ...
    did   = {c: [] for c in conds}   
    dropD = {c: [] for c in conds}   
    dropR = {c: [] for c in conds}  
    for sd in seeds:
